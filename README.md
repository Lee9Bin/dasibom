...
- 공식 코드표 기준 전국 252개 시군구 검색과 8개 에디터 추천 지역
- 지역명·시도·테마 검색, 검증된 중심 좌표 지도와 목록 탐색
- 실시간 TourAPI 사진 갤러리, 원본 이미지 생존 여부 검증, 촬영자·촬영월·출처, 사진 더 보기
- 지역 관광지와 지도 링크, 향후 집중률과 낮은 날짜 3개 추천
- 브라우저 여행 저장, 익명 좋아요/취소, 공유, 지역별 Open Graph
- 화면 요청 시 공공 API 실시간 호출, 공공 API 응답 DB 저장·장기 캐시 미사용

전국 17개 지표·전국 순위·숨은 점수와 정책 배지는 아직 제공하지 않습니다. 전국 지역 탐색과 8개 지역의 편집 선정을 구분해 표시하며 빈 API 응답을 임의의 점수나 장소로 채우지 않습니다.
