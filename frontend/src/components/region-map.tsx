...
export default function RegionMap({regions}:{regions:Region[]}){
 ...
 const mapped=regions.filter(r=>r.latitude!==null&&r.longitude!==null);const bounds=new maplibregl.LngLatBounds();mapped.forEach(r=>{const link=document.createElement("a");link.className="map-pin";link.href=`/regions/${r.code}`;link.textContent=r.name;link.setAttribute("aria-label",`${r.name} 상세 보기`);new maplibregl.Marker({element:link}).setLngLat([r.longitude!,r.latitude!]).addTo(map!);bounds.extend([r.longitude!,r.latitude!]);});if(mapped.length>1)map.fitBounds(bounds,{padding:70,maxZoom:8,duration:0});
