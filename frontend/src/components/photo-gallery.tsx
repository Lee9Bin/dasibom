...
export function PhotoGallery({photos}:{photos:Photo[]}) {
  ...
  return <><div className="photo-grid">{photos.slice(0,count).map(p=><figure key={p.id}>
    ...
    <figcaption><strong>{p.title}</strong><span>{p.source} · {p.photographer||"촬영자 정보 없음"}</span><span>{p.location}</span><span>{/^\d{6}$/.test(p.month)?`${p.month.slice(0,4)}.${p.month.slice(4)} 촬영`:"촬영월 미제공"}</span></figcaption>
